  ...
  function readClass() {
    ...
    if (q < 5 || q < 0) {
      ...
      if (index > -1) {
        ...
        removed["name"].push(
          record["q" + q]["name"].splice(index, 1).toString()
        );
        removed["givenGrade"].push(
          parseInt(record["q" + q]["givenGrade"].splice(index, 1))
        );
        removed["maxGrade"].push(
          parseInt(record["q" + q]["maxGrade"].splice(index, 1))
        );
        removed["weight"].push(
          parseInt(record["q" + q]["weight"].splice(index, 1))
        );
        removed["class"].push(
          parseInt(record["q" + q]["class"].splice(index, 1))
        );
      }
