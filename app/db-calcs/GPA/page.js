...
export default async function Page(){
    ...
    function getMaj(q){
        ...
        if (index > -1) {
            ...
            perc = (parseInt(record["data"][q]["givenGrade"].splice(index, 1)))/(parseInt(record["data"][q]["maxGrade"].splice(index, 1)))
        }
        if (index > -1) {
        percs.push(perc)
        }
    ...
    for (let step = 0; step < record.data.length; step++) {
        if(record.data.length - 1 == step){
            ...
            MajPerc = (getMaj(step).reduce((accumulator, currentValue) => accumulator + currentValue, 0))/getMaj(step).length
        }
    }
    //-------------------------------------------
    let percs2 = [];
    ...
    for (let step = 0; step < record.data.length; step++) {
        if(record.data.length - 1 == step){
            ...
            MinPerc = (getMin(step).reduce((accumulator, currentValue) => accumulator + currentValue, 0))/getMin(step).length
        }
    }
    finalPerc = ((Math.round(MajPerc*100))*3+(Math.round(MinPerc*100))*7)/10
